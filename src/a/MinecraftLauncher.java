...
public class MinecraftLauncher extends AbstractMinecraftLauncher {
	...
	public String[] buildJavaCommand() {
		...
		if (this.validateJavaLaunchCommand(stringArray)) {
			...
			case WINDOWS: {
				...
				stringArray = new String[] { file.getPath(), "--tmpDir", System.getProperty("java.io.tmpdir"),
			...
			case OSX: {
				...
				stringArray = new String[] { string, "--tmpDir", System.getProperty("java.io.tmpdir"), "--workDir",
	...
	private boolean validateJavaLaunchCommand(String[] var1) {
		if (var1 != null && var1.length >= 5) {
			return var1[0].equals("java") && var1[1].equals("-jar") && var1[2].equals("launcher.jar")
					&& var1[3].equals("--workDir") && var1[4].equals(".");
		} else {
			return false;
		}
	}
	...
	public void f() {
		...
		if (var1.a() != Severity.none) {
			GamebandDialog infoDialog = new GamebandDialog("Getting the latest Minecraft Launcher",
					"This won't take long...", "", true);
	...
	private boolean n() {
		if (OS.getOS() == OS.WINDOWS) {
			...
			if (!var2.exists()) {
				GamebandPopup.PopupDialog((Component) null, (new String[] {
						"We're having problems downloading Minecraft :(",
						"This is usually due to being blocked by antivirus or firewall software. Please add Gameband to your antivirus software's whitelist.",
						"Alternatively, you can manually download the minecraft.exe file and copy it into the minecraft folder on Gameband:" }),
						"Gameband Error", "https://launcher.mojang.com/download/Minecraft.exe");
